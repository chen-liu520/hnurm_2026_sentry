...
// 辅助函数：将目标ID字符串转换为TargetType枚举值
static uint8_t idToTargetType(const std::string& id) {
  if (id == "hero") {
    return hnurm_interfaces::msg::TargetType::HERO;
  } else if (id == "engineer") {
    return hnurm_interfaces::msg::TargetType::ENGINEER;
  } else if (id == "infantry3" || id == "3") {
    return hnurm_interfaces::msg::TargetType::INFANTRY3;
  } else if (id == "infantry4" || id == "4") {
    return hnurm_interfaces::msg::TargetType::INFANTRY4;
  } else if (id == "infantry5" || id == "5") {
    return hnurm_interfaces::msg::TargetType::INFANTRY5;
  } else if (id == "outpost") {
    return hnurm_interfaces::msg::TargetType::OUTPOST;
  } else if (id == "sentry") {
    return hnurm_interfaces::msg::TargetType::SENTRY;
  } else if (id == "base") {
    return hnurm_interfaces::msg::TargetType::BASE;
  } else {
    return hnurm_interfaces::msg::TargetType::NONE;
  }
}
...
: Node("armor_solver", options), solver_(nullptr) {
  ...
  measure_pub_ = this->create_publisher<hnurm_interfaces::msg::Measurement>("armor_solver/measurement",
                                                                         rclcpp::SensorDataQoS());

  // Publisher
  target_pub_ = this->create_publisher<hnurm_interfaces::msg::Target>("armor_solver/target",
                                                                   rclcpp::SensorDataQoS());
  gimbal_pub_ = this->create_publisher<hnurm_interfaces::msg::VisionSendData>("vision_send_data",
                                                                      rclcpp::SensorDataQoS());
  // Timer 250 Hz
  ...
  enable_ = true;
  set_mode_srv_ = this->create_service<hnurm_interfaces::srv::SetMode>(
...
void ArmorSolverNode::timerCallback() {
  ...
  // Init message
  hnurm_interfaces::msg::VisionSendData control_msg;

  // If target never detected
  if (armor_target_.header.frame_id.empty()) {
    control_msg.yaw_diff = 0;
    control_msg.pitch_diff = 0;
    control_msg.target_distance = -1;
    control_msg.pitch = 0;
    control_msg.yaw = 0;
    control_msg.target_state.data = hnurm_interfaces::msg::TargetState::LOST_TARGET;
    control_msg.target_type.data  = hnurm_interfaces::msg::TargetType::NONE;
  ...
  if (armor_target_.tracking) {
    try {
      ...
      // 使用辅助函数将目标ID转换为TargetType
      control_msg.target_type.data = idToTargetType(armor_target_.id);
    } catch (...) {
...
void ArmorSolverNode::armorsCallback(const hnurm_interfaces::msg::Armors::SharedPtr armors_msg) {
  ...
  armors_msg->armors.erase(std::remove_if(armors_msg->armors.begin(),
                                          ...
                                          [](const hnurm_interfaces::msg::Armor &armor) {
                                            return abs(armor.pose.position.z) > 2;
                                          }),
                           armors_msg->armors.end());

  // Init message
  hnurm_interfaces::msg::Measurement measure_msg;
...
void ArmorSolverNode::publishMarkers(const hnurm_interfaces::msg::Target &target_msg,
  ...
  if (target_msg.tracking) {
    ...
    selection_marker_.pose.position.y = gimbal_cmd.target_distance * sin(gimbal_cmd.yaw * M_PI / 180);
    selection_marker_.pose.position.x = gimbal_cmd.target_distance * cos(gimbal_cmd.yaw * M_PI / 180);
    selection_marker_.pose.position.z = gimbal_cmd.target_distance * sin(gimbal_cmd.pitch * M_PI / 180);
    ...
    if (gimbal_cmd.target_state.data != hnurm_interfaces::msg::TargetState::LOST_TARGET) {
...
void ArmorSolverNode::setModeCallback(
  const std::shared_ptr<hnurm_interfaces::srv::SetMode::Request> request,
