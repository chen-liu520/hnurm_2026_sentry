...
#include "hnurm_utils/math/utils.hpp"

namespace fyt::auto_aim {
Solver::Solver(std::weak_ptr<rclcpp::Node> n) : node_(n) {
  auto node = node_.lock();

  shooting_range_w_ = node->declare_parameter("solver.shooting_range_width", 0.150);
...
hnurm_interfaces::msg::VisionSendData Solver::solve(const hnurm_interfaces::msg::Target &target,
  ...
  Eigen::Vector3d base_position(target.position.x, target.position.y, target.position.z);
  double base_yaw = target.yaw;
  double flying_time = trajectory_compensator_->getFlyingTime(base_position);
  double dt = 0;
  Eigen::Vector3d target_position = base_position;
  double target_yaw = base_yaw;
  for (int i = 0; i < 3; i++) {
    // 总延迟 = (当前时间 - 视觉戳时间) + 预测弹道飞行时间 + 预设延迟补偿
    dt = (current_time - rclcpp::Time(target.header.stamp)).seconds() + flying_time + prediction_delay_;
    
    // 更新预测位置：每次都从 base_position 开始累加，防止多次迭代导致误差叠加
    target_position.x() = base_position.x() + dt * target.velocity.x;
    target_position.y() = base_position.y() + dt * target.velocity.y;
    target_position.z() = base_position.z() + dt * target.velocity.z;
    target_yaw = base_yaw + dt * target.v_yaw;

    // 根据新预测出的位置（距离和高度变了），重新获取更准确的飞行时间
    double new_flying_time = trajectory_compensator_->getFlyingTime(target_position);
    
    // 如果时间变化极小（小于1ms），说明位置和时间已经闭环，提前跳出
    if (std::abs(new_flying_time - flying_time) < 0.001) {
      break;
    }
  ...
  hnurm_interfaces::msg::VisionSendData gimbal_cmd;
  gimbal_cmd.header = target.header;
  gimbal_cmd.target_distance = distance;
  if(isOnTarget(rpy_[2], rpy_[1], yaw, pitch, distance)) {
    gimbal_cmd.target_state.data = hnurm_interfaces::msg::TargetState::FIRE;
  } else {
    gimbal_cmd.target_state.data = hnurm_interfaces::msg::TargetState::CONVERGING;
  }
  ...
  switch (state) {
    case TRACKING_ARMOR: {
      ...
      if (controller_delay_ != 0) {
        ...
        gimbal_cmd.target_distance = chosen_armor_position.norm();
        if (chosen_armor_position.norm() < 0.1) {
          throw std::runtime_error("No valid armor to shoot");
        }
        calcYawAndPitch(chosen_armor_position, rpy_, yaw, pitch);
  ...
  if (gimbal_cmd.target_state.data == hnurm_interfaces::msg::TargetState::FIRE) {
