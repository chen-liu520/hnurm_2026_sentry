...
#include "hnurm_interfaces/msg/debug_armors.hpp"
#include "hnurm_interfaces/msg/debug_lights.hpp"
...
public:
  ...
  void initYOLO (const std::string & model_path);
  ...
  // Debug msgs
  cv::Mat binary_img;
  hnurm_interfaces::msg::DebugLights debug_lights;
  hnurm_interfaces::msg::DebugArmors debug_armors;
...
private:
  ...
  std::vector<Armor> detectYolo(const cv::Mat & input) noexcept;
  inline double sigmoid(double x) { return x > 0 ? 1.0 / (1.0 + exp(-x)) : exp(x) / (1.0 + exp(x)); }
