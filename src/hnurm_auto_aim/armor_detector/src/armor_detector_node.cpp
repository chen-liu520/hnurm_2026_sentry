...
#include "hnurm_utils/assert.hpp"
#include "hnurm_utils/common.hpp"
#include "hnurm_utils/logger/log.hpp"
#include "hnurm_utils/math/pnp_solver.hpp"
...
ArmorDetectorNode::ArmorDetectorNode(const rclcpp::NodeOptions &options)
  ...
  // Armors Publisher
  armors_pub_ = this->create_publisher<hnurm_interfaces::msg::Armors>(
  ...
  // target_sub_ = this->create_subscription<hnurm_interfaces::msg::Target>(
  //   "armor_solver/target",
...
std::unique_ptr<Detector> ArmorDetectorNode::initDetector() {
  ...
  auto detector = std::make_unique<Detector>(binary_thres, EnemyColor::RED,
                                             l_params, a_params);
  detector->use_yolo = use_yolo_;
  std::string yolo_model_path = "package://armor_detector/model/yolo.xml";
  if (detector->use_yolo) {
        std::string resolved_path = utils::URLResolver::getResolvedPath(yolo_model_path);
    FYT_INFO("armor_detector", "YOLO mode ON. Loading: {}", resolved_path);
    detector->initYOLO(resolved_path);
  }
...
ArmorDetectorNode::onSetParameters(std::vector<rclcpp::Parameter> parameters) {
  ...
  for (const auto &param : parameters) {
    if (param.get_name() == "use_yolo") {
      detector_->use_yolo = param.as_bool();
      FYT_WARN("armor_detector", "Mode Switched: %s", detector_->use_yolo ? "YOLO" : "TRADITIONAL");
...
// hnurm_interfaces::msg::Target::SharedPtr target_msg) {
...
void ArmorDetectorNode::createDebugPublishers() noexcept {
  lights_data_pub_ = this->create_publisher<hnurm_interfaces::msg::DebugLights>(
      "armor_detector/debug_lights", 10);
  armors_data_pub_ = this->create_publisher<hnurm_interfaces::msg::DebugArmors>(
