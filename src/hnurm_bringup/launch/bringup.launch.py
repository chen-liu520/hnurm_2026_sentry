...
sys.path.append(os.path.join(get_package_share_directory('hnurm_bringup'), 'launch'))
...
def generate_launch_description():
    ...
    launch_params = yaml.safe_load(open(os.path.join(
        get_package_share_directory('hnurm_bringup'), 'config', 'launch_params.yaml')))

    SetParameter(name='rune',value=launch_params['rune']),
    robot_gimbal_description = Command(['xacro ', os.path.join(
        get_package_share_directory('hnurm_robot_description'), 'urdf', 'rm_gimbal.urdf.xacro'),
    ...
    robot_navigation_description = Command(['xacro ', os.path.join(
        get_package_share_directory('hnurm_robot_description'), 'urdf', 'sentry.urdf.xacro')])
    ...
    def get_params(name):
        return os.path.join(get_package_share_directory('hnurm_bringup'), 'config', 'node_params', '{}_params.yaml'.format(name))

    # 图像
    if launch_params['video_play']: 
        image_node  = ComposableNode(
            package='hnurm_camera_driver',
        ...
        image_node  = ComposableNode(
            ...
            name='camera_node',
            parameters=[get_params('hnurm_camera')],
            extra_arguments=[{'use_intra_process_comms': True}]
        )
    ...
    if launch_params['virtual_serial']:
        ...
        serial_driver_node = Node(
            ...
            output='both',
            emulate_tty=True,
            parameters=[get_params('hnurm_uart')],
