...
| enable_angle_filter | Bool | Enable angle-sector filtering for point publishing<br>true -- remove points inside [angle_filter_min_deg, angle_filter_max_deg] in XY plane<br>false -- disable filtering | false |
| angle_filter_min_deg | Float | Start angle (degree) of filtered sector in XY plane, based on atan2(y, x), valid with enable_angle_filter=true | -30.0 |
| angle_filter_max_deg | Float | End angle (degree) of filtered sector in XY plane, supports cross-180 range when min > max | 30.0 |
...
```json
{
  ...
  "lidar_configs" : [
    ...
    },
    {
      ...
      "angle_filter_min_deg" : -30.0,
      "angle_filter_max_deg" : 30.0,
      "extrinsic_parameter" : {
        "roll": 0.0,
        "pitch": 0.0,
        "yaw": 0.0,
