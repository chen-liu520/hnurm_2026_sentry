...
1. 创建空的3D点云对象
2. 通过投影器`projectLaser`将`LaserScan`直接转换到`PointCloud2`。可以理解为极坐标转换为直角坐标，只是换了一种消息类型，点云的形状不会改变，还是"二维平面"
3. 将3D点云坐标转换至目标坐标系
...
### `dummy_pointcloud_publisher.cpp`
...
### `sample_laserscan_to_pointcloud.launch.py`

将2D激光扫描消息转换为3D点云，这里2D激光扫描消息是直接通过launch文件发布的模拟数据，用于测试2D->3D的转换

![alt text](<截图 2026-04-14 21-57-34.png>)

### `sample_pointcloud_to_laserscan.launch.py`

启动`dummy_pointcloud_publisher.cpp`生成随机的3D点云，同时发布了一个`map->cloud`的tf变换，模拟目标坐标系的变化
再将3D点云转换为2D激光扫描消息，用于测试3D->2D的转换

![alt text](<截图 2026-04-14 21-22-18.png>)